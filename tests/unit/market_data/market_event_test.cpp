...
[[nodiscard]] model::Price parse_price_or_throw(std::string_view value) {
...
[[nodiscard]] model::Quantity parse_quantity_or_throw(std::string_view value) {
...
[[nodiscard]] const runtime::RuntimeSource& configured_runtime_source_or_throw() {
...
[[nodiscard]] market_data::MarketSourceIdentity create_source_identity_or_throw() {
  return market_data::MarketSourceIdentity::from_runtime_source(
      configured_runtime_source_or_throw());
}

// --------------------------------------------------------
// Validate an alternate source ID against the same sealed market tuple before copying its identity.
[[nodiscard]] market_data::MarketSourceIdentity
create_source_identity_from_id_or_throw(std::string_view source_id) {
  auto configuration_result = configuration::StartupConfiguration::create_startup_configuration(
      test_support::create_reference_configuration_params_or_throw());
  if (!configuration_result) {
...
// tests.
[[nodiscard]] market_data::NormalizedMarketUpdateFields create_update_fields(
    std::vector<market_data::MarketLevelChange> changes,
    market_data::MarketUpdateKind kind = market_data::MarketUpdateKind::Delta,
    model::ReceiveSequence receive_sequence = model::ReceiveSequence::create_initial(),
  ...
  return market_data::NormalizedMarketUpdateFields{
      create_source_identity_or_throw(),
...
[[nodiscard]] std::vector<market_data::MarketLevelChange> create_representative_changes_or_throw() {
  return {
      {market_data::BookSide::Ask, parse_price_or_throw("101.50"), parse_quantity_or_throw("2.0")},
      {market_data::BookSide::Bid, parse_price_or_throw("100.00"), parse_quantity_or_throw("3.5")},
      {market_data::BookSide::Ask, parse_price_or_throw("101.00"), parse_quantity_or_throw("1.0")}};
}
...
[[nodiscard]] market_data::NormalizedMarketUpdate create_normalized_update_or_throw(
    market_data::IntegrityVerdict verdict = market_data::IntegrityVerdict::Accepted) {
  auto result = market_data::NormalizedMarketUpdate::create_normalized_market_update(
      create_update_fields(create_representative_changes_or_throw(),
                           market_data::MarketUpdateKind::Delta,
                           model::ReceiveSequence::create_initial(), model::ReceiveTimestamp{50U},
                           create_market_integrity_or_throw(verdict)),
      8U);
  if (!result) {
...
derive_normalized_digest_or_throw(market_data::NormalizedMarketUpdateFields fields) {
  auto result =
      market_data::NormalizedMarketUpdate::create_normalized_market_update(std::move(fields), 8U);
  if (!result) {
...
create_state_event_fields(market_data::MarketReadiness readiness,
                          std::optional<market_data::MarketReadiness> previous = std::nullopt) {
  return market_data::MarketStateEventFields{create_source_identity_or_throw(),
                                             std::nullopt,
                                             std::nullopt,
                                             std::nullopt,
                                             std::nullopt,
...
TEST_CASE("M2 nominal counters fail before zero entry or unsigned wrap",
  ...
  const auto generation_overflow = maximum_generation.value().derive_next_ordinal();
  const auto callback_overflow = maximum_callback.value().derive_next_ordinal();
  REQUIRE_FALSE(generation_overflow);
  REQUIRE_FALSE(callback_overflow);
  CHECK(generation_overflow.error() ==
        model::DomainError::create_at_field(model::DomainErrorCode::MarketCounterExhausted,
                                            "book_generation"));
...
TEST_CASE("normalized updates derive a deterministic timing-independent payload digest",
  ...
  auto authored = create_representative_changes_or_throw();
  auto reversed = authored;
  std::reverse(reversed.begin(), reversed.end());

  const auto first = market_data::NormalizedMarketUpdate::create_normalized_market_update(
      create_update_fields(std::move(authored)), 8U);
  const auto second = market_data::NormalizedMarketUpdate::create_normalized_market_update(
      create_update_fields(std::move(reversed), market_data::MarketUpdateKind::Delta,
                           create_ordinal_or_throw<model::ReceiveSequence>(9U),
                           model::ReceiveTimestamp{900U}),
      8U);
  ...
  CHECK(first.value().changes()[1U].price == parse_price_or_throw("101"));
  CHECK(first.value().changes()[2U].price == parse_price_or_throw("101.5"));
  ...
  auto changed_fields = create_update_fields(create_representative_changes_or_throw());
  changed_fields.source_timestamp = model::SourceTimestamp{1'001U};
  const auto changed = market_data::NormalizedMarketUpdate::create_normalized_market_update(
      std::move(changed_fields), 8U);
  REQUIRE(changed);
  ...
  const auto baseline_fields = create_update_fields(create_representative_changes_or_throw());
  const auto baseline_digest = derive_normalized_digest_or_throw(baseline_fields);
  CHECK(digest_to_hex(baseline_digest) ==
  ...
  const auto check_changed = [&](market_data::NormalizedMarketUpdateFields fields) {
    CHECK_FALSE(derive_normalized_digest_or_throw(std::move(fields)) == baseline_digest);
  };
  ...
  changed_source.source = create_source_identity_from_id_or_throw("source.alternate-deribit-btc");
  ...
  changed_token.integrity =
      create_market_integrity_or_throw(market_data::IntegrityVerdict::Accepted, "other-token");
  ...
  changed_price.changes[0U].price = parse_price_or_throw("101.75");
  check_changed(std::move(changed_price));
  auto changed_quantity = baseline_fields;
  changed_quantity.changes[0U].quantity = parse_quantity_or_throw("2.5");
  ...
  changed_receive_sequence.receive_sequence = create_ordinal_or_throw<model::ReceiveSequence>(9U);
  CHECK(derive_normalized_digest_or_throw(std::move(changed_receive_sequence)) == baseline_digest);
  auto changed_receive_time = baseline_fields;
  changed_receive_time.receive_timestamp = model::ReceiveTimestamp{900U};
  CHECK(derive_normalized_digest_or_throw(std::move(changed_receive_time)) == baseline_digest);
...
TEST_CASE("normalized updates reject invalid shape and preserve stable errors",
  ...
  auto zero_source_fields = create_update_fields(create_representative_changes_or_throw());
  zero_source_fields.source_sequence = model::SequenceNumber{0U};
  const auto zero_source = market_data::NormalizedMarketUpdate::create_normalized_market_update(
      std::move(zero_source_fields), 8U);
  REQUIRE_FALSE(zero_source);
  CHECK(zero_source.error() ==
        model::DomainError::create_at_field(model::DomainErrorCode::InvalidMarketEvent,
                                            "market_update.source_sequence"));

  auto zero_predecessor_fields = create_update_fields(create_representative_changes_or_throw());
  zero_predecessor_fields.predecessor_sequence = model::SequenceNumber{0U};
  const auto zero_predecessor =
      market_data::NormalizedMarketUpdate::create_normalized_market_update(
          std::move(zero_predecessor_fields), 8U);
  ...
  auto invalid_kind_fields = create_update_fields(create_representative_changes_or_throw());
  invalid_kind_fields.kind = static_cast<market_data::MarketUpdateKind>(std::uint8_t{99U});
  const auto invalid_kind = market_data::NormalizedMarketUpdate::create_normalized_market_update(
      std::move(invalid_kind_fields), 8U);
  REQUIRE_FALSE(invalid_kind);
  CHECK(invalid_kind.error() ==
        model::DomainError::create_at_field(model::DomainErrorCode::InvalidMarketEvent,
                                            "market_update.kind"));

  auto invalid_integrity_fields = create_update_fields(create_representative_changes_or_throw());
  ...
  const auto invalid_integrity =
      market_data::NormalizedMarketUpdate::create_normalized_market_update(
          std::move(invalid_integrity_fields), 8U);
  ...
  const auto oversized = market_data::NormalizedMarketUpdate::create_normalized_market_update(
      create_update_fields(create_representative_changes_or_throw()), 2U);
  REQUIRE_FALSE(oversized);
  CHECK(oversized.error() ==
        model::DomainError::create_at_field(model::DomainErrorCode::MarketBookCapacityExceeded,
                                            "market_update.changes"));

  const auto invalid_compiled_bound =
      market_data::NormalizedMarketUpdate::create_normalized_market_update(
          create_update_fields(create_representative_changes_or_throw()),
          market_data::maximum_changes_per_market_update + 1U);
  ...
  auto invalid_side_fields = create_update_fields(create_representative_changes_or_throw());
  invalid_side_fields.changes[0U].side = static_cast<market_data::BookSide>(std::uint8_t{99U});
  const auto invalid_side = market_data::NormalizedMarketUpdate::create_normalized_market_update(
      std::move(invalid_side_fields), 8U);
  ...
  const auto duplicate = market_data::NormalizedMarketUpdate::create_normalized_market_update(
      create_update_fields(
          {{market_data::BookSide::Bid, parse_price_or_throw("100"), parse_quantity_or_throw("1")},
           {market_data::BookSide::Bid, parse_price_or_throw("100.0"),
  ...
  const auto deletion = market_data::NormalizedMarketUpdate::create_normalized_market_update(
      create_update_fields({{market_data::BookSide::Ask, parse_price_or_throw("101"),
                             parse_quantity_or_throw("0")}}),
      8U);
  REQUIRE(deletion);

  const auto snapshot_zero = market_data::NormalizedMarketUpdate::create_normalized_market_update(
      create_update_fields(
  ...
  const auto negative_price = market_data::NormalizedMarketUpdate::create_normalized_market_update(
      create_update_fields(
          {{market_data::BookSide::Bid, parse_price_or_throw("-1"), parse_quantity_or_throw("1")}}),
      8U);
...
TEST_CASE("normalized market commands preserve distinct deterministic control types",
  ...
  const market_data::SessionStarted session{
      create_source_identity_or_throw(), model::SessionEpoch{8U}, model::SourceTimestamp{2'000U},
      create_ordinal_or_throw<model::ReceiveSequence>(2U), model::ReceiveTimestamp{70U}};
  const market_data::StalenessCheck freshness{
      create_source_identity_or_throw(), model::SessionEpoch{8U},
      create_ordinal_or_throw<model::ReceiveSequence>(3U), model::ReceiveTimestamp{75U},
      model::ProcessingTimestamp{80U}};
  ...
  const market_data::NormalizedRecordedMarketCommand update_command{
      create_normalized_update_or_throw()};
...
TEST_CASE("MarketStateEvent validates sanitized readiness transitions",
  ...
  const auto initial = market_data::validate_market_state_transition(
      create_state_event_fields(market_data::MarketReadiness::Synchronizing));
  REQUIRE(initial);

  // ++++++++++++++++++++++++++++++++++++++++
  // Explicit owner resynchronization alone may publish Synchronizing while already there.
  const auto owner_resynchronization = market_data::validate_market_state_transition(
      create_state_event_fields(market_data::MarketReadiness::Synchronizing,
                                market_data::MarketReadiness::Synchronizing));
  REQUIRE(owner_resynchronization);

  auto envelope_same_state = create_state_event_fields(market_data::MarketReadiness::Synchronizing,
                                                       market_data::MarketReadiness::Synchronizing);
  envelope_same_state.session_epoch = model::SessionEpoch{7U};
  envelope_same_state.receive_sequence = model::ReceiveSequence::create_initial();
  envelope_same_state.receive_timestamp = model::ReceiveTimestamp{50U};
  envelope_same_state.admission_ordinal = model::AdmissionOrdinal::create_initial();
  ...
  auto ready_fields = create_state_event_fields(market_data::MarketReadiness::Ready,
                                                market_data::MarketReadiness::Synchronizing);
  ready_fields.session_epoch = model::SessionEpoch{7U};
  ready_fields.source_sequence = model::SequenceNumber{101U};
  ready_fields.receive_sequence = model::ReceiveSequence::create_initial();
  ready_fields.receive_timestamp = model::ReceiveTimestamp{50U};
  ready_fields.admission_ordinal = model::AdmissionOrdinal::create_initial();
  ready_fields.metadata_revision = model::InstrumentMetadataRevision::create_initial();
  ready_fields.book_generation = model::BookGeneration::create_initial();
  ready_fields.book_revision = create_ordinal_or_throw<model::BookRevision>(3U);
  ...
  const auto invalid_initial = market_data::validate_market_state_transition(
      create_state_event_fields(market_data::MarketReadiness::Invalid));
  REQUIRE_FALSE(invalid_initial);
  CHECK(invalid_initial.error().context.field == "market_state.previous_readiness");

  const auto unchanged = market_data::validate_market_state_transition(create_state_event_fields(
      market_data::MarketReadiness::Stale, market_data::MarketReadiness::Stale));
  ...
  auto unknown_fields = create_state_event_fields(market_data::MarketReadiness::Ready,
                                                  market_data::MarketReadiness::Synchronizing);
  ...
  auto partial_receive = create_state_event_fields(market_data::MarketReadiness::Invalid,
                                                   market_data::MarketReadiness::Ready);
  partial_receive.receive_sequence = model::ReceiveSequence::create_initial();
  partial_receive.book_generation = model::BookGeneration::create_initial();
  partial_receive.book_revision = model::BookRevision::create_initial();
  ...
  auto regressing = create_state_event_fields(market_data::MarketReadiness::Invalid,
                                              market_data::MarketReadiness::Ready);
  regressing.session_epoch = model::SessionEpoch{7U};
  regressing.receive_sequence = model::ReceiveSequence::create_initial();
  regressing.receive_timestamp = model::ReceiveTimestamp{61U};
  regressing.admission_ordinal = model::AdmissionOrdinal::create_initial();
  regressing.book_generation = model::BookGeneration::create_initial();
  regressing.book_revision = model::BookRevision::create_initial();
  ...
  auto incoherent_book = create_state_event_fields(market_data::MarketReadiness::Invalid,
                                                   market_data::MarketReadiness::Ready);
  incoherent_book.admission_ordinal = model::AdmissionOrdinal::create_initial();
  incoherent_book.book_generation = create_ordinal_or_throw<model::BookGeneration>(2U);
  incoherent_book.book_revision = model::BookRevision::create_initial();
  ...
  auto initial_with_book = create_state_event_fields(market_data::MarketReadiness::Synchronizing);
  initial_with_book.book_generation = model::BookGeneration::create_initial();
  initial_with_book.book_revision = model::BookRevision::create_initial();
  ...
  auto stale_fields = create_state_event_fields(market_data::MarketReadiness::Stale,
                                                market_data::MarketReadiness::Ready);
  stale_fields.session_epoch = model::SessionEpoch{7U};
  stale_fields.receive_sequence = create_ordinal_or_throw<model::ReceiveSequence>(2U);
  stale_fields.receive_timestamp = model::ReceiveTimestamp{50U};
  stale_fields.admission_ordinal = create_ordinal_or_throw<model::AdmissionOrdinal>(2U);
  stale_fields.book_generation = model::BookGeneration::create_initial();
  stale_fields.book_revision = create_ordinal_or_throw<model::BookRevision>(3U);
  ...
  auto precommit_discontinuity = create_state_event_fields(
      market_data::MarketReadiness::Invalid, market_data::MarketReadiness::Synchronizing);
  precommit_discontinuity.admission_ordinal = create_ordinal_or_throw<model::AdmissionOrdinal>(3U);
  ...
  fence_with_receive.receive_sequence = create_ordinal_or_throw<model::ReceiveSequence>(2U);
