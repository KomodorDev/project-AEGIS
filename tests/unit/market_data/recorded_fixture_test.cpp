...
[[nodiscard]] model::ReceiveSequence create_receive_sequence_or_throw(std::uint64_t value) {
...
[[nodiscard]] market_data::IngressFrameAttempt create_ingress_attempt_or_throw(
    std::string frame, std::optional<std::string_view> source_id = "source.deribit.public") {
  ...
  auto result = market_data::IngressFrameAttempt::create_ingress_frame_attempt(
      std::move(typed_source), model::SessionEpoch{7U}, std::move(frame));
  if (!result) {
...
create_policy_with_limits_or_throw(std::uint32_t maximum_changes = static_cast<std::uint32_t>(
                                       market_data::maximum_changes_per_market_update),
                                   std::uint32_t maximum_frame_bytes = static_cast<std::uint32_t>(
                                       market_data::maximum_recorded_frame_bytes),
...
[[nodiscard]] const runtime::RuntimePolicy& configured_runtime_policy_or_throw() {
  static const auto policy = create_policy_with_limits_or_throw();
  return policy;
}

// --------------------------------------------------------
// Mint the immutable post-admission frame from policy and explicitly owner-assigned receipt fields.
[[nodiscard]] market_data::RecordedFrame create_recorded_frame_or_throw(
    std::string frame, const runtime::RuntimePolicy& policy = configured_runtime_policy_or_throw(),
    std::uint64_t sequence = 19U, std::uint64_t receive_time = 9'000U,
    std::optional<std::string_view> source_id = "source.deribit.public") {
  auto result = market_data::RecordedFrame::create_recorded_frame(
      create_ingress_attempt_or_throw(std::move(frame), source_id), policy,
      create_receive_sequence_or_throw(sequence), model::ReceiveTimestamp{receive_time});
  if (!result) {
...
void require_recorded_fixture_parse_error(
    std::string frame, market_data::RecordedFixtureParseCode code, std::size_t byte_offset,
    std::string_view attributed_source = "source.deribit.public") {
  const auto result = market_data::parse_recorded_fixture(create_recorded_frame_or_throw(
      std::move(frame), configured_runtime_policy_or_throw(), 19U, 9'000U, attributed_source));
...
normalize_recorded_frame_or_throw_on_parse_error(
    std::string frame, const runtime::RuntimePolicy& policy = configured_runtime_policy_or_throw(),
    std::uint64_t sequence = 19U, std::uint64_t receive_time = 9'000U,
    std::string_view source_id = "source.deribit.public") {
  auto parsed = market_data::parse_recorded_fixture(
      create_recorded_frame_or_throw(std::move(frame), policy, sequence, receive_time, source_id));
  if (!parsed) {
...
[[nodiscard]] std::string create_maximum_level_frame() {
...
TEST_CASE("recorded fixture source identity is nominal and exactly bounded",
  ...
  const auto maximum_source_policy = create_policy_with_limits_or_throw(
  ...
  const auto accepted = market_data::parse_recorded_fixture(create_recorded_frame_or_throw(
      maximum_frame, maximum_source_policy, 19U, 9'000U, maximum_source));
  REQUIRE(accepted);
  CHECK(accepted.value().source().source_id() ==
        parse_identifier_or_throw<model::MarketSourceId>(maximum_source));
  ...
  require_recorded_fixture_parse_error(underscore,
                                       market_data::RecordedFixtureParseCode::InvalidSourceId,
  ...
  require_recorded_fixture_parse_error(oversized_frame,
                                       market_data::RecordedFixtureParseCode::InvalidSourceId,
...
TEST_CASE("recorded snapshot parsing preserves attribution and exact book data",
  ...
  const auto result = market_data::parse_recorded_fixture(create_recorded_frame_or_throw(
      "AEGISMD|1|source.deribit.public|snapshot|100|none|5000000|2|ok:book100|2|"
  ...
  CHECK(result.value().source().source_id() ==
        parse_identifier_or_throw<model::MarketSourceId>("source.deribit.public"));
  CHECK(result.value().source().source_ordinal() == model::MarketSourceOrdinal::create_initial());
  CHECK(result.value().source().venue_id() == parse_identifier_or_throw<model::VenueId>("deribit"));
  CHECK(result.value().source().instrument_id() ==
        parse_identifier_or_throw<model::InstrumentId>("BTC-USD-PERPETUAL"));
  CHECK(result.value().source().venue_instrument_id() ==
        parse_identifier_or_throw<model::VenueInstrumentId>("BTC-PERPETUAL"));
  CHECK(result.value().session_epoch() == model::SessionEpoch{7U});
  CHECK(result.value().receive_sequence() == create_receive_sequence_or_throw(19U));
...
TEST_CASE("session and staleness fixture controls normalize deterministically",
  ...
  const auto session = normalize_recorded_frame_or_throw_on_parse_error(
      "AEGISMD|1|source.deribit.public|session-started|7000", configured_runtime_policy_or_throw(),
      21U, 9'100U);
  REQUIRE(session);
  const auto& session_command = std::get<market_data::SessionStarted>(session.value());
  CHECK(session_command.source.source_id() ==
        parse_identifier_or_throw<model::MarketSourceId>("source.deribit.public"));
  CHECK(session_command.source.source_ordinal() == model::MarketSourceOrdinal::create_initial());
  ...
  CHECK(session_command.receive_sequence == create_receive_sequence_or_throw(21U));
  ...
  const auto staleness = normalize_recorded_frame_or_throw_on_parse_error(
      "AEGISMD|1|source.deribit.public|staleness-check|12000", configured_runtime_policy_or_throw(),
      22U, 9'200U);
  REQUIRE(staleness);
  const auto& staleness_command = std::get<market_data::StalenessCheck>(staleness.value());
  CHECK(staleness_command.source.source_ordinal() == model::MarketSourceOrdinal::create_initial());
  CHECK(staleness_command.receive_sequence == create_receive_sequence_or_throw(22U));
...
TEST_CASE("recorded fixtures reject unsupported contracts and source mismatch",
  ...
  require_recorded_fixture_parse_error(unsupported_version,
                                       market_data::RecordedFixtureParseCode::UnsupportedVersion,
                                       unsupported_version.find('2'));

  const std::string unsupported_type = "AEGISMD|1|source.deribit.public|quote|1";
  require_recorded_fixture_parse_error(
      unsupported_type, market_data::RecordedFixtureParseCode::UnsupportedMessageType,
  ...
  require_recorded_fixture_parse_error(source_mismatch,
                                       market_data::RecordedFixtureParseCode::SourceMismatch,
...
TEST_CASE("recorded fixtures report truncation and level-count mismatch",
  ...
  require_recorded_fixture_parse_error(
      truncated, market_data::RecordedFixtureParseCode::UnexpectedEnd, truncated.size());

  const std::string missing_level =
      "AEGISMD|1|source.deribit.public|snapshot|1|none|2|1|ok:x|2|B,100,1";
  require_recorded_fixture_parse_error(missing_level,
                                       market_data::RecordedFixtureParseCode::LevelCountMismatch,
                                       missing_level.size());
}
...
TEST_CASE("recorded fixtures reject invalid numeric and level syntax",
  ...
  require_recorded_fixture_parse_error(
      signed_sequence, market_data::RecordedFixtureParseCode::InvalidUnsignedInteger,
  ...
  require_recorded_fixture_parse_error(overflow_sequence,
                                       market_data::RecordedFixtureParseCode::NumericOverflow,
  ...
  require_recorded_fixture_parse_error(zero_revision,
                                       market_data::RecordedFixtureParseCode::InvalidRevision,
  ...
  require_recorded_fixture_parse_error(shifted_level,
                                       market_data::RecordedFixtureParseCode::InvalidLevelSyntax,
                                       shifted_level.find("B,100"));

  const std::string exponent_price = "AEGISMD|1|source.deribit.public|delta|2|1|3|1|ok:x|1|B,1e3,1";
  require_recorded_fixture_parse_error(exponent_price,
                                       market_data::RecordedFixtureParseCode::InvalidPrice,
                                       exponent_price.find("1e3"));

  const std::string negative_quantity =
      "AEGISMD|1|source.deribit.public|delta|2|1|3|1|ok:x|1|B,100,-1";
  require_recorded_fixture_parse_error(negative_quantity,
                                       market_data::RecordedFixtureParseCode::InvalidQuantity,
...
TEST_CASE("recorded fixture frame, level, and integrity bounds are exact",
  ...
  require_recorded_fixture_parse_error(boundary_frame,
                                       market_data::RecordedFixtureParseCode::InvalidMagic, 0U);
  ...
  const auto oversized_attempt = market_data::IngressFrameAttempt::create_ingress_frame_attempt(
      parse_identifier_or_throw<model::MarketSourceId>("source.deribit.public"),
      model::SessionEpoch{7U}, oversized_frame);
  ...
  auto maximum_levels = market_data::parse_recorded_fixture(
      create_recorded_frame_or_throw(create_maximum_level_frame()));
  ...
  const auto maximum_normalized = market_data::normalize_recorded_fixture(
      std::move(maximum_levels).value(), configured_runtime_policy_or_throw());
  ...
  require_recorded_fixture_parse_error(too_many_levels,
                                       market_data::RecordedFixtureParseCode::TooManyLevels,
  ...
  REQUIRE(market_data::parse_recorded_fixture(create_recorded_frame_or_throw(maximum_integrity)));
  REQUIRE(normalize_recorded_frame_or_throw_on_parse_error(maximum_integrity));
  ...
  require_recorded_fixture_parse_error(oversized_integrity,
                                       market_data::RecordedFixtureParseCode::InvalidIntegrityToken,
...
TEST_CASE("configured frame limit accepts its exact edge and rejects the next byte",
  ...
  const auto policy = create_policy_with_limits_or_throw(
      static_cast<std::uint32_t>(market_data::maximum_changes_per_market_update), configured_limit);
  auto exact = market_data::IngressFrameAttempt::create_ingress_frame_attempt(
      parse_identifier_or_throw<model::MarketSourceId>("source.deribit.public"),
      model::SessionEpoch{7U}, std::string(configured_limit, 'x'));
  REQUIRE(exact);
  const auto exact_source = market_data::resolve_recorded_frame_source(exact.value(), policy);
  REQUIRE(exact_source);
  CHECK(exact_source.value() == model::MarketSourceOrdinal::create_initial());
  const auto exact_frame = market_data::RecordedFrame::create_recorded_frame(
      std::move(exact).value(), policy, create_receive_sequence_or_throw(1U),
      model::ReceiveTimestamp{10U});
  REQUIRE(exact_frame);
  ...
  auto over = market_data::IngressFrameAttempt::create_ingress_frame_attempt(
      parse_identifier_or_throw<model::MarketSourceId>("source.deribit.public"),
      model::SessionEpoch{7U}, std::string(configured_limit + 1U, 'x'));
  REQUIRE(over);
  ...
  CHECK(rejected_source.error() ==
        model::DomainError::create_at_field(model::DomainErrorCode::InvalidMarketEvent,
                                            "ingress_frame_attempt.frame"));
  const auto rejected_frame = market_data::RecordedFrame::create_recorded_frame(
      std::move(over).value(), policy, create_receive_sequence_or_throw(2U),
      model::ReceiveTimestamp{11U});
...
TEST_CASE("recorded fixtures reject trailing input", "[market_data][recorded_fixture]") {
  ...
  require_recorded_fixture_parse_error(frame, market_data::RecordedFixtureParseCode::TrailingInput,
                                       frame.find("extra"));
}
...
TEST_CASE("syntax failures stay separate from normalized market validation",
  ...
  require_recorded_fixture_parse_error(
      malformed, market_data::RecordedFixtureParseCode::InvalidQuantity, malformed.find("broken"));

  // ++++++++++++++++++++++++++++++++++++++++
  // A syntactically valid zero snapshot quantity is rejected only by normalized update semantics.
  const auto zero_snapshot = normalize_recorded_frame_or_throw_on_parse_error(
      "AEGISMD|1|source.deribit.public|snapshot|1|none|2|1|ok:x|1|B,100,0");
  ...
  const auto zero_source = normalize_recorded_frame_or_throw_on_parse_error(
      "AEGISMD|1|source.deribit.public|snapshot|0|none|2|1|ok:x|1|B,100,1");
  REQUIRE_FALSE(zero_source);
  CHECK(zero_source.error() ==
        model::DomainError::create_at_field(model::DomainErrorCode::InvalidMarketEvent,
                                            "market_update.source_sequence"));

  const auto zero_predecessor = normalize_recorded_frame_or_throw_on_parse_error(
      "AEGISMD|1|source.deribit.public|delta|2|0|3|1|ok:x|1|B,100,1");
  ...
  const auto duplicate = normalize_recorded_frame_or_throw_on_parse_error(
      "AEGISMD|1|source.deribit.public|delta|2|1|3|1|ok:x|2|B,100,1|B,100,2");
  ...
  const auto constrained_policy = create_policy_with_limits_or_throw(1U);
  const auto constrained = normalize_recorded_frame_or_throw_on_parse_error(
...
TEST_CASE("policy resolution rejects missing and unconfigured ingress attribution",
  ...
  auto missing = market_data::IngressFrameAttempt::create_ingress_frame_attempt(
      std::nullopt, model::SessionEpoch{7U}, "AEGISMD|1|source.deribit.public|session-started|7");
  REQUIRE(missing);
  const auto missing_source = market_data::resolve_recorded_frame_source(
      missing.value(), configured_runtime_policy_or_throw());
  ...
  const auto missing_frame = market_data::RecordedFrame::create_recorded_frame(
      std::move(missing).value(), configured_runtime_policy_or_throw(),
      create_receive_sequence_or_throw(1U), model::ReceiveTimestamp{10U});
  ...
  auto unconfigured = create_ingress_attempt_or_throw(
      "AEGISMD|1|source.other.public|session-started|7", "source.other.public");
  const auto unconfigured_source = market_data::resolve_recorded_frame_source(
      unconfigured, configured_runtime_policy_or_throw());
  REQUIRE_FALSE(unconfigured_source);
  CHECK(unconfigured_source.error() ==
        model::DomainError::create_at_field(model::DomainErrorCode::RuntimeSourceNotConfigured,
                                            "ingress_frame_attempt.source_id"));
  const auto unconfigured_frame = market_data::RecordedFrame::create_recorded_frame(
      std::move(unconfigured), configured_runtime_policy_or_throw(),
      create_receive_sequence_or_throw(2U), model::ReceiveTimestamp{11U});
  ...
  auto parsed = market_data::parse_recorded_fixture(create_recorded_frame_or_throw(
      "AEGISMD|1|source.deribit.public|session-started|7", configured_runtime_policy_or_throw()));
  REQUIRE(parsed);
  const auto other_policy = create_policy_with_limits_or_throw(
      static_cast<std::uint32_t>(market_data::maximum_changes_per_market_update),
...
TEST_CASE("fixture normalization canonicalizes changes and reproduces one payload digest",
  ...
  const auto first = normalize_recorded_frame_or_throw_on_parse_error(
      "AEGISMD|1|source.deribit.public|delta|101|100|5000100|1|ok:book101|2|"
      "A,30001,2|B,30000,3",
      configured_runtime_policy_or_throw(), 19U, 9'000U);
  const auto second = normalize_recorded_frame_or_throw_on_parse_error(
      "AEGISMD|1|source.deribit.public|delta|101|100|5000100|1|ok:book101|2|"
      "B,30000,3|A,30001,2",
      configured_runtime_policy_or_throw(), 20U, 9'500U);
  ...
  CHECK(first_update.source().source_id() ==
        parse_identifier_or_throw<model::MarketSourceId>("source.deribit.public"));
  CHECK(first_update.source().source_ordinal() == model::MarketSourceOrdinal::create_initial());
  ...
  CHECK(digest_to_hex(first_update.payload_digest()) ==
...
TEST_CASE("malformed fixture parsing does not corrupt subsequent parsing or normalization",
  ...
  const auto before = market_data::parse_recorded_fixture(create_recorded_frame_or_throw(valid));
  REQUIRE(before);

  const auto malformed = market_data::parse_recorded_fixture(create_recorded_frame_or_throw(
      "AEGISMD|1|source.deribit.public|delta|2|1|3|1|ok:x|1|A,101,broken"));
  ...
  const auto after = market_data::parse_recorded_fixture(create_recorded_frame_or_throw(valid));
  REQUIRE(after);
  CHECK(after.value() == before.value());

  auto first_message = before.value();
  auto second_message = after.value();
  const auto first_normalized = market_data::normalize_recorded_fixture(
      std::move(first_message), configured_runtime_policy_or_throw());
  const auto second_normalized = market_data::normalize_recorded_fixture(
      std::move(second_message), configured_runtime_policy_or_throw());
