...
TEST_CASE("M4 provenance resolver publishes every configuration-proved subject shape",
  ...
  const auto root_only = resolver.root_only();
  ...
  const auto account = resolver.configured_account(fixture.account_id(), fixture.venue_id());
  REQUIRE(account);
  ...
  REQUIRE(account_subject.firm_id().has_value());
  ...
  const auto instrument = resolver.configured_instrument(fixture.account_id(), fixture.venue_id(),
...
TEST_CASE("M4 authoritative provenance remains source-limited", "[m4][provenance]") {
  ...
  const auto supported = resolver.authoritative_source(fixture.account_id(), fixture.venue_id(),
                                                       fixture.instrument_id());
  REQUIRE(supported.subject().has_value());
  REQUIRE(supported.subject()->firm_id().has_value());
  ...
  const auto wrong_venue =
      resolver.authoritative_source(fixture.account_id(), other_venue, fixture.instrument_id());
  ...
  const auto unknown_account =
      test_support::m4_id<model::LogicalAccountId>("account.unknown-private-source");
  const auto unknown =
      resolver.authoritative_source(unknown_account, fixture.venue_id(), fixture.instrument_id());
...
TEST_CASE("M4 configured provenance rejects missing or contradictory account authority",
  ...
  const auto& resolver = fixture.resolver();
  const auto unknown_account =
      test_support::m4_id<model::LogicalAccountId>("account.not-configured");
  const auto wrong_venue = test_support::m4_id<model::VenueId>("other");

  const auto missing = resolver.configured_account(unknown_account, fixture.venue_id());
  REQUIRE_FALSE(missing);
  REQUIRE(missing.error().context.field == "m4_provenance.logical_account_id");

  const auto contradictory = resolver.configured_account(fixture.account_id(), wrong_venue);
  REQUIRE_FALSE(contradictory);
  REQUIRE(contradictory.error().context.field == "m4_provenance.venue_id");

  const auto unsupported =
      resolver.configured_instrument(fixture.account_id(), fixture.venue_id(),
...
TEST_CASE("M4 provenance resolver rejects a different sealed configuration", "[m4][provenance]") {
  const auto authority = test_support::m4_test_authority();
  auto other =
...
TEST_CASE("M4 provenance resolver owns its complete normalization authority", "[m4][provenance]") {
  ...
  auto resolver = [] {
    auto authority = test_support::m4_test_authority();
    auto created =
  ...
  const auto account =
      test_support::m4_id<model::LogicalAccountId>("account.deribit-testnet-aegis");
  const auto venue = test_support::m4_id<model::VenueId>("deribit");
  const auto instrument = test_support::m4_id<model::InstrumentId>("BTC-USD-PERPETUAL");
  const auto provenance = resolver.configured_instrument(account, venue, instrument);
  REQUIRE(provenance);
  REQUIRE(provenance.value().subject()->firm_id().has_value());
