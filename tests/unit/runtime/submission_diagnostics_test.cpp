...
[[nodiscard]] runtime::SubmissionDiagnosticFields create_attempt_fields_or_throw() {
  runtime::SubmissionDiagnosticFields fields;
  fields.attempt_id = create_identity_or_throw<model::SubmissionAttemptId>(3U);
  fields.owner_turn_ordinal = create_identity_or_throw<model::TurnOrdinal>(5U);
  fields.callback_ordinal = create_identity_or_throw<model::CallbackOrdinal>(7U);
  return fields;
}
...
TEST_CASE("submission diagnostics enforce fixed profiles and provenance",
  ...
  const runtime::SubmissionDiagnosticProvenance provenance{create_digest(std::byte{0x11U}),
  ...
  auto evidence = create_attempt_fields_or_throw();
  evidence.stage = execution::SubmissionStage::Evidence;
  evidence.reason = execution::SubmissionReason::EvidenceCapacityExceeded;
  REQUIRE(sink.validate_diagnostic(runtime::SubmissionDiagnosticKind::EvidenceCapacityExceeded,
                                   evidence));
  CHECK(sink.diagnostic_count() == 0U);
  REQUIRE(sink.append_diagnostic(runtime::SubmissionDiagnosticKind::EvidenceCapacityExceeded,
                                 evidence));
  ...
  auto reentry = create_attempt_fields_or_throw();
  ...
  REQUIRE(sink.append_diagnostic(runtime::SubmissionDiagnosticKind::ReentryDetected, reentry));
  auto repeated_reentry = reentry;
  repeated_reentry.occurrence_count = std::numeric_limits<std::uint64_t>::max() - 3U;
  REQUIRE(
      sink.append_diagnostic(runtime::SubmissionDiagnosticKind::ReentryDetected, repeated_reentry));
  repeated_reentry.occurrence_count = 1U;
  REQUIRE(
      sink.append_diagnostic(runtime::SubmissionDiagnosticKind::ReentryDetected, repeated_reentry));

  auto measurement = create_attempt_fields_or_throw();
  ...
  REQUIRE(sink.append_diagnostic(runtime::SubmissionDiagnosticKind::MeasurementUnavailable,
                                 measurement));

  CHECK(sink.accepted_count() == 3U);
  REQUIRE(sink.diagnostic_at(0U) != nullptr);
  CHECK(sink.diagnostic_at(0U)->ordinal == 1U);
  CHECK(sink.diagnostic_at(0U)->provenance == provenance);
  REQUIRE(sink.diagnostic_at(1U) != nullptr);
  CHECK(sink.diagnostic_at(1U)->fields.occurrence_count ==
  ...
  CHECK_FALSE(
      sink.append_diagnostic(runtime::SubmissionDiagnosticKind::EvidenceCapacityExceeded, partial));
  auto zero_occurrence = reentry;
  zero_occurrence.occurrence_count = 0U;
  CHECK_FALSE(
      sink.append_diagnostic(runtime::SubmissionDiagnosticKind::ReentryDetected, zero_occurrence));
  CHECK_FALSE(
      sink.append_diagnostic(static_cast<runtime::SubmissionDiagnosticKind>(999U), reentry));
  CHECK(sink.accepted_count() == 3U);
...
TEST_CASE("submission diagnostics retain release and uncertainty observations before saturation",
          "[runtime][diagnostics][submission][m3]") {
  const runtime::SubmissionDiagnosticProvenance provenance{create_digest(std::byte{0x41U}),
                                                           create_digest(std::byte{0x42U}),
                                                           create_digest(std::byte{0x43U})};
  ...
  auto released = create_attempt_fields_or_throw();
  auto order_provider_result =
  ...
  auto order_result = order_provider.generate_next_order_id();
  REQUIRE(order_result);
  released.order_id = std::move(order_result).value();
  released.reservation_id = create_identity_or_throw<model::ReservationId>(3U);
  released.stage = execution::SubmissionStage::Encoding;
  released.reason = execution::SubmissionReason::EncodingFailed;
  REQUIRE(sink.append_diagnostic(runtime::SubmissionDiagnosticKind::ReservationReleased, released));
  ...
  REQUIRE(
      sink.append_diagnostic(runtime::SubmissionDiagnosticKind::UnknownExposureRetained, retained));
  ...
  REQUIRE(sink.diagnostic_at(0U) != nullptr);
  REQUIRE(sink.diagnostic_at(1U) != nullptr);
  CHECK(sink.diagnostic_at(0U)->kind == runtime::SubmissionDiagnosticKind::ReservationReleased);
  CHECK(sink.diagnostic_at(1U)->kind == runtime::SubmissionDiagnosticKind::UnknownExposureRetained);
  CHECK(sink.diagnostic_at(2U) == nullptr);

  auto invalid = retained;
  invalid.reservation_id.reset();
  REQUIRE_FALSE(
      sink.append_diagnostic(runtime::SubmissionDiagnosticKind::UnknownExposureRetained, invalid));
  CHECK(sink.dropped_count() == 2U);
