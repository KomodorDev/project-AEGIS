...
configuration_from(configuration::StartupConfigurationParams params) {
  auto result = configuration::StartupConfiguration::create(std::move(params));
  if (!result) {
...
[[nodiscard]] runtime::RuntimePolicyLimits reference_limits() {
...
reference_source(std::string_view source_id = "source.deribit-btc-perpetual") {
...
[[nodiscard]] runtime::RuntimePolicyParams reference_policy_params() {
  return runtime::RuntimePolicyParams{reference_limits(), {reference_source()}};
}
...
[[nodiscard]] configuration::StartupConfigurationParams two_source_configuration_params() {
  ...
  params.subscriptions.push_back(market_data::Subscription{
      id<model::SubscriptionId>("subscription.kraken-eth-perpetual-book"), params.bots.front().id,
      venue_id, instrument_id, market_data::SubscriptionChannel::OrderBook});
  return params;
...
[[nodiscard]] runtime::RuntimePolicyParams two_source_policy_params() {
  auto params = reference_policy_params();
  params.sources.front().source_id = id<model::MarketSourceId>("source.z-deribit-btc");
  params.sources.push_back(runtime::RuntimeSourceDefinition{
      id<model::MarketSourceId>("source.a-kraken-eth"),
      id<model::VenueId>("kraken"),
...
[[nodiscard]] configuration::StartupConfigurationParams two_callback_configuration_params() {
  auto params = test_support::two_firm_configuration_params();
  params.subscriptions.push_back(market_data::Subscription{
      id<model::SubscriptionId>("subscription.subsidiary-deribit-btc-book"),
...
TEST_CASE("runtime policy seals positive bounds and configured source provenance",
  ...
  const auto result = runtime::RuntimePolicy::create(configuration, reference_policy_params());

  REQUIRE(result);
  const auto& policy = result.value();
  CHECK(policy.configuration_fingerprint() == configuration.fingerprint());
  CHECK(policy.limits() == reference_limits());
  ...
  const auto* const by_id =
      policy.find_source(id<model::MarketSourceId>("source.deribit-btc-perpetual"));
  REQUIRE(by_id != nullptr);
  const auto key = runtime::RuntimeSourceKey{id<model::VenueId>("deribit"),
                                             id<model::InstrumentId>("BTC-USD-PERPETUAL"),
                                             market_data::SubscriptionChannel::OrderBook};
  CHECK(policy.find_source(key) == by_id);
  CHECK(policy.find_source(id<model::MarketSourceId>("source.missing")) == nullptr);
  ...
  const auto peer_configuration = configuration_from(test_support::two_firm_configuration_params());
  const auto peer_policy =
      runtime::RuntimePolicy::create(peer_configuration, reference_policy_params());
  REQUIRE(peer_policy);
...
TEST_CASE("runtime policy source order is canonical and fingerprinted stably",
  ...
  auto first_params = two_source_policy_params();
  auto second_params = first_params;
  std::reverse(second_params.sources.begin(), second_params.sources.end());

  const auto first = runtime::RuntimePolicy::create(configuration, std::move(first_params));
  const auto second = runtime::RuntimePolicy::create(configuration, std::move(second_params));
  ...
  CHECK(first.value().sources()[0U].definition().source_id ==
        id<model::MarketSourceId>("source.a-kraken-eth"));
  CHECK(first.value().sources()[0U].ordinal() == model::MarketSourceOrdinal::initial());
  CHECK(first.value().sources()[1U].definition().source_id ==
        id<model::MarketSourceId>("source.z-deribit-btc"));
...
TEST_CASE("every runtime policy limit participates independently in canonical identity",
  ...
  const auto configuration = configuration_from(test_support::reference_configuration_params());
  const auto baseline = runtime::RuntimePolicy::create(configuration, reference_policy_params());
  REQUIRE(baseline);

  const auto check_changed_limit = [&](runtime::RuntimePolicyLimits limits) {
    const auto changed = runtime::RuntimePolicy::create(
        configuration, runtime::RuntimePolicyParams{limits, {reference_source()}});
    REQUIRE(changed);
...
TEST_CASE("every runtime source member participates in canonical identity",
  ...
  const auto baseline_configuration =
      configuration_from(test_support::reference_configuration_params());
  const auto baseline =
      runtime::RuntimePolicy::create(baseline_configuration, reference_policy_params());
  REQUIRE(baseline);
  ...
  const auto check_changed_source =
      [&](configuration::StartupConfigurationParams configuration_params,
        ...
        const auto changed_configuration = configuration_from(std::move(configuration_params));
        const auto changed = runtime::RuntimePolicy::create(
            changed_configuration, runtime::RuntimePolicyParams{reference_limits(), {source}});
        REQUIRE(changed);
        REQUIRE(changed.value().sources().size() == 1U);
        CHECK(changed.value().sources().front().definition() == source);
        CHECK(changed.value().sources().front().definition() != reference_source());
        CHECK(changed.value().canonical_bytes() != baseline.value().canonical_bytes());
        CHECK(changed.value().fingerprint() != baseline.value().fingerprint());
  ...
  SECTION("source ID") {
    auto source = reference_source();
    source.source_id = id<model::MarketSourceId>("source.deribit-btc-perpetual-secondary");
    check_changed_source(test_support::reference_configuration_params(), source);
  }

  // ++++++++++++++++++++++++++++++++++++++++
  // Venue identity must stay coherent across metadata, observation, account, and route catalogs.
  SECTION("venue ID") {
    auto configuration_params = test_support::reference_configuration_params();
    const auto venue_id = id<model::VenueId>("kraken");
  ...
  SECTION("instrument ID") {
    auto configuration_params = test_support::reference_configuration_params();
    const auto instrument_id = id<model::InstrumentId>("XBT-USD-PERPETUAL");
  ...
  SECTION("venue instrument ID") {
    auto configuration_params = test_support::reference_configuration_params();
    const auto venue_instrument_id = id<model::VenueInstrumentId>("XBT-PERPETUAL");
...
TEST_CASE("runtime policy rejects duplicate source identities and keys", "[runtime][policy][m2]") {
  ...
  SECTION("duplicate source identity") {
    auto params = two_source_policy_params();
    params.sources[1U].source_id = params.sources[0U].source_id;
    const auto result = runtime::RuntimePolicy::create(configuration, std::move(params));
  ...
  SECTION("duplicate observation key") {
    ...
    duplicate.source_id = id<model::MarketSourceId>("source.second-deribit-btc");
...
TEST_CASE("runtime policy rejects dangling or mismatched source definitions",
  ...
  SECTION("venue is not configured") {
    auto params = reference_policy_params();
    params.sources.front().venue_id = id<model::VenueId>("kraken");
    const auto result = runtime::RuntimePolicy::create(configuration, std::move(params));
    REQUIRE_FALSE(result);
    CHECK(result.error() ==
          model::DomainError::at_index(model::DomainErrorCode::RuntimeSourceNotConfigured,
                                       "runtime_policy.sources.venue_id", 0U));
  }

  SECTION("instrument is not configured at the venue") {
    auto params = reference_policy_params();
    params.sources.front().instrument_id = id<model::InstrumentId>("ETH-USD-PERPETUAL");
    const auto result = runtime::RuntimePolicy::create(configuration, std::move(params));
    REQUIRE_FALSE(result);
    CHECK(result.error() ==
          model::DomainError::at_index(model::DomainErrorCode::RuntimeSourceNotConfigured,
  ...
  SECTION("venue-native instrument identity disagrees") {
    auto params = reference_policy_params();
    params.sources.front().venue_instrument_id = id<model::VenueInstrumentId>("BTC-29AUG26");
    const auto result = runtime::RuntimePolicy::create(configuration, std::move(params));
    REQUIRE_FALSE(result);
    CHECK(result.error() ==
          model::DomainError::at_index(model::DomainErrorCode::InvalidRuntimePolicy,
  ...
  SECTION("no order-book subscription grants observation") {
    ...
    const auto unsubscribed_configuration = configuration_from(std::move(configuration_params));
    const auto result =
        runtime::RuntimePolicy::create(unsubscribed_configuration, reference_policy_params());
    REQUIRE_FALSE(result);
    CHECK(result.error() ==
          model::DomainError::at_index(model::DomainErrorCode::RuntimeSourceNotConfigured,
...
TEST_CASE("runtime policy rejects zero and over-limit bounds", "[runtime][policy][m2]") {
  ...
  const auto check_failure = [&](runtime::RuntimePolicyLimits limits, std::string_view field) {
    const auto result = runtime::RuntimePolicy::create(
        configuration, runtime::RuntimePolicyParams{limits, {reference_source()}});
    REQUIRE_FALSE(result);
    CHECK(result.error() == model::DomainError::at_field(
...
TEST_CASE("runtime policy requires sources and sufficient callback fanout capacity",
  ...
  SECTION("source registry is empty") {
    ...
    CHECK(result.error() == model::DomainError::at_field(model::DomainErrorCode::EmptyCollection,
                                                         "runtime_policy.sources"));
  }

  SECTION("recovery snapshot callback fanout exceeds policy capacity") {
    const auto multi_callback_configuration =
        configuration_from(two_callback_configuration_params());
    auto params = reference_policy_params();
    params.limits.maximum_callbacks_per_turn = 2U;
    const auto result =
        runtime::RuntimePolicy::create(multi_callback_configuration, std::move(params));
    REQUIRE_FALSE(result);
    CHECK(result.error() ==
          model::DomainError::at_index(model::DomainErrorCode::InvalidRuntimePolicy,
  ...
  SECTION("single-grant trace capacity must cover the maximum recovery turn") {
    ...
    const auto insufficient = runtime::RuntimePolicy::create(configuration, params);
    ...
    const auto boundary = runtime::RuntimePolicy::create(configuration, std::move(params));
    REQUIRE(boundary);
  ...
  SECTION("multi-grant trace capacity uses overflow-safe exact fanout") {
    ...
    const auto insufficient = runtime::RuntimePolicy::create(multi_callback_configuration, params);
    REQUIRE_FALSE(insufficient);
    CHECK(insufficient.error() ==
          model::DomainError::at_field(model::DomainErrorCode::InvalidRuntimePolicy,
                                       "runtime_policy.runtime_trace_capacity"));
