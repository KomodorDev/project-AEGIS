...
[[nodiscard]] model::Result<Value> create_bot_runtime_failure_result(DomainErrorCode code,
                                                                     std::string_view field) {
  return model::Result<Value>::create_failure(
      DomainError::create_at_field(code, std::string{field}));
}

// --------------------------------------------------------
// Convert strategy-visible readiness into the independent canonical trace vocabulary.
[[nodiscard]] trace::RuntimeMarketState
runtime_trace_state_from_market_readiness(market_data::MarketReadiness readiness) {
  switch (readiness) {
...
template <typename GrantType>
[[nodiscard]] trace::RuntimeTraceFields
create_state_callback_trace_fields(const GrantType& grant,
                                   const market_data::MarketStateEvent& event,
  ...
  fields.previous_state =
      source.previous_readiness
          ? runtime_trace_state_from_market_readiness(source.previous_readiness.value())
          : trace::RuntimeMarketState::Unspecified;
  fields.state = runtime_trace_state_from_market_readiness(source.readiness);
  return fields;
}
...
template <typename GrantType>
[[nodiscard]] trace::RuntimeTraceFields
create_market_callback_trace_fields(const GrantType& grant, const market_data::MarketEvent& event,
...
template <typename GrantType>
[[nodiscard]] trace::RuntimeTraceFields
create_reentry_trace_fields(const GrantType& grant, model::TurnOrdinal turn_ordinal,
                            model::CallbackOrdinal callback_ordinal) {
  trace::RuntimeTraceFields fields;
  fields.bot_id = grant.attribution.bot_id;
  fields.subscription_id = grant.subscription.id;
...
execution::SubmitResult BotContext::submit_order(const execution::OrderRequest& request) noexcept {
  ...
  if (active_token == 0U) {
    return execution::SubmitResult::create_locally_rejected_result(
        execution::SubmissionStage::Context, execution::SubmissionReason::ContextInactive);
  }
  if (active_token != current_token) {
    return execution::SubmitResult::create_locally_rejected_result(
        execution::SubmissionStage::Context, execution::SubmissionReason::WrongOwner);
  }
  return submission_coordinator_->submit_order(
...
model::Result<BotRuntime> BotRuntime::create_bot_runtime(
  ...
  if (policy.configuration_fingerprint() != configuration.fingerprint() ||
    ...
    return create_bot_runtime_failure_result<BotRuntime>(DomainErrorCode::InvalidRelationship,
                                                         "bot_runtime.provenance");
  }
  ...
  if (submission_coordinator != nullptr &&
    ...
    return create_bot_runtime_failure_result<BotRuntime>(
        DomainErrorCode::InvalidRelationship, "bot_runtime.submission_coordinator_provenance");
  }
  ...
  for (std::size_t index = 0U; index < registrations.size(); ++index) {
    if (!registrations[index].strategy) {
      return create_bot_runtime_failure_result<BotRuntime>(DomainErrorCode::StrategyNotConfigured,
  ...
  std::vector<SubscriptionCallbackGrant> grants;
  ...
  for (const auto& source : policy.sources()) {
    ...
    for (const auto& subscription : configuration.subscriptions().subscriptions()) {
      ...
      grants.push_back(SubscriptionCallbackGrant{
          source.ordinal(), trace::RuntimeTraceSource::from_runtime_source(source), subscription,
    ...
    if (matching_count != static_cast<std::size_t>(source.matching_subscription_count())) {
      return create_bot_runtime_failure_result<BotRuntime>(
          DomainErrorCode::InvalidRelationship, "bot_runtime.grants.matching_subscription_count");
    }
  ...
  auto created = model::Result<BotRuntime>::create_success(BotRuntime{
...
BotRuntime::preflight_dispatch_callbacks(model::MarketSourceOrdinal source_ordinal,
  ...
  if (!healthy) {
    return model::Result<BotDispatchPlan>::create_failure(healthy.error());
  }
  if (dispatch_active_) {
    return create_bot_runtime_failure_result<BotDispatchPlan>(
        DomainErrorCode::DispatchReentryDetected, "bot_runtime.preflight_reentry");
  }
  ...
  if (source_index + 1U >= source_offsets_.size()) {
    return create_bot_runtime_failure_result<BotDispatchPlan>(
        DomainErrorCode::RuntimeSourceNotConfigured, "bot_runtime.source_ordinal");
  }
  if (event_count > 2U) {
    return create_bot_runtime_failure_result<BotDispatchPlan>(DomainErrorCode::InvalidValue,
  ...
  if (matching > static_cast<std::size_t>(std::numeric_limits<std::uint32_t>::max())) {
    return create_bot_runtime_failure_result<BotDispatchPlan>(
        DomainErrorCode::DispatchCapacityExceeded, "bot_runtime.matching_subscriptions");
  }
  const auto callback_count = static_cast<std::uint64_t>(matching) * event_count;
  if (callback_count > maximum_callbacks_per_turn_) {
    return create_bot_runtime_failure_result<BotDispatchPlan>(
        DomainErrorCode::DispatchCapacityExceeded, "bot_runtime.callback_count");
  }
  ...
  if (callback_count > std::numeric_limits<std::uint64_t>::max() - last) {
    return create_bot_runtime_failure_result<BotDispatchPlan>(
        DomainErrorCode::CallbackCounterExhausted, "callback_ordinal");
  }
  ...
  if (callback_trace_records > std::numeric_limits<std::uint32_t>::max()) {
    return create_bot_runtime_failure_result<BotDispatchPlan>(
        DomainErrorCode::DispatchCapacityExceeded, "bot_runtime.callback_trace_records");
  }
  auto trace_capacity =
      trace_sink_->preflight_trace_append(static_cast<std::uint32_t>(callback_trace_records));
  if (!trace_capacity) {
    return model::Result<BotDispatchPlan>::create_failure(trace_capacity.error());
  }
  if (completed_dispatch_count_ == std::numeric_limits<std::uint64_t>::max()) {
    return create_bot_runtime_failure_result<BotDispatchPlan>(
        DomainErrorCode::CounterExhausted, "bot_runtime.completed_dispatch_count");
  }
  ...
  return model::Result<BotDispatchPlan>::create_success(
...
model::Result<void> BotRuntime::require_healthy() const {
  if (status_.canonical_trace_failure_latched) {
    return create_bot_runtime_failure_result<void>(DomainErrorCode::RuntimeEvidenceExhausted,
                                                   "bot_runtime.canonical_trace_evidence");
  }
  if (status_.callback_clock_regression_latched) {
    return create_bot_runtime_failure_result<void>(DomainErrorCode::InvalidTimestampOrder,
...
void BotRuntime::cancel_dispatch_callbacks(const BotDispatchPlan& plan) noexcept {
...
model::Result<void> BotRuntime::validate_plan(const BotDispatchPlan& plan) const {
  if (plan.owner_ != this) {
    return create_bot_runtime_failure_result<void>(DomainErrorCode::InvalidRelationship,
  ...
  if (plan.callback_counter_predecessor_ != last_callback_ordinal_) {
    return create_bot_runtime_failure_result<void>(
        DomainErrorCode::InvalidRelationship, "bot_dispatch_plan.callback_counter_predecessor");
  }

  const auto source_index = static_cast<std::size_t>(plan.source_ordinal_.value() - 1U);
  if (source_index + 1U >= source_offsets_.size()) {
    return create_bot_runtime_failure_result<void>(DomainErrorCode::RuntimeSourceNotConfigured,
...
model::Result<BotDispatchReport> BotRuntime::reject_reentry() {
  ...
  if (!recorded) {
    return model::Result<BotDispatchReport>::create_failure(recorded.error());
  }
  return create_bot_runtime_failure_result<BotDispatchReport>(
      DomainErrorCode::DispatchReentryDetected, "bot_runtime.dispatch_reentry");
}
...
model::Result<void> BotRuntime::record_reentry(bool owner_reentry) noexcept {
  ...
  if (active_grant_ == nullptr || active_prepared_callback_ == nullptr || !active_turn_ordinal_ ||
    ...
    return create_bot_runtime_failure_result<void>(error_code, error_field);
  }
  ...
  if (active_reentry_attempts_ == std::numeric_limits<std::uint64_t>::max()) {
    return create_bot_runtime_failure_result<void>(DomainErrorCode::CounterExhausted,
                                                   "bot_runtime.reentry_count");
  }
  ...
  if (!active_reentry_traced_ && !active_reentry_trace_failed_) {
    ...
    auto appended = trace_sink_->append_trace_record(
    ...
    if (!appended) {
      ...
      return model::Result<void>::create_failure(appended.error());
    }
...
BotRuntime::dispatch_callbacks(const BotDispatchPlan& plan,
                               const market_data::MarketTurnOutcome& outcome) {
  if (dispatch_active_) {
  ...
  auto healthy = require_healthy();
  if (!healthy) {
    return model::Result<BotDispatchReport>::create_failure(healthy.error());
  }
  auto valid_plan = validate_plan(plan);
  if (!valid_plan) {
    return model::Result<BotDispatchReport>::create_failure(valid_plan.error());
  }
  if (outcome.source_ordinal() != plan.source_ordinal_) {
    return create_bot_runtime_failure_result<BotDispatchReport>(
        DomainErrorCode::InvalidRelationship, "bot_runtime.outcome_source_plan");
  }
  if (outcome.turn_ordinal() != plan.turn_ordinal_) {
    return create_bot_runtime_failure_result<BotDispatchReport>(
        DomainErrorCode::InvalidRelationship, "bot_runtime.outcome_turn_plan");
  }
  ...
  if (event_count != plan.event_count_) {
    return create_bot_runtime_failure_result<BotDispatchReport>(
        DomainErrorCode::DispatchCapacityExceeded, "bot_runtime.outcome_event_count");
  }
  ...
  if (exact_callbacks != plan.callback_count_ ||
    ...
    return create_bot_runtime_failure_result<BotDispatchReport>(
        DomainErrorCode::DispatchCapacityExceeded, "bot_runtime.outcome_callback_count");
  }
  if (!has_state && !has_market) {
    ++completed_dispatch_count_;
    return model::Result<BotDispatchReport>::create_success(
  ...
  if (!source_ordinal) {
    return create_bot_runtime_failure_result<BotDispatchReport>(
        DomainErrorCode::RuntimeSourceNotConfigured, "bot_runtime.outcome_source");
  }
  if (!turn_ordinal) {
    return create_bot_runtime_failure_result<BotDispatchReport>(
        DomainErrorCode::InvalidRelationship, "bot_runtime.outcome_turn");
  }
  if (source_ordinal.value() != outcome.source_ordinal()) {
    return create_bot_runtime_failure_result<BotDispatchReport>(
        DomainErrorCode::InvalidRelationship, "bot_runtime.outcome_event_source");
  }
  if (turn_ordinal.value() != outcome.turn_ordinal()) {
    return create_bot_runtime_failure_result<BotDispatchReport>(
        DomainErrorCode::InvalidRelationship, "bot_runtime.outcome_event_turn");
  }
  if (has_market && !outcome.ready_book()) {
    return create_bot_runtime_failure_result<BotDispatchReport>(DomainErrorCode::MarketNotReady,
                                                                "bot_runtime.ready_book");
  }

  auto capacity = trace_sink_->preflight_trace_append(outcome.reserved_callback_trace_records());
  if (!capacity) {
    return model::Result<BotDispatchReport>::create_failure(capacity.error());
  }
  ...
  const auto prepare = [&](SubscriptionCallbackGrant& grant,
                           bool state_callback) -> model::Result<void> {
    auto ordinal = predicted ? predicted->derive_next_ordinal()
                             : model::Result<model::CallbackOrdinal>::create_success(
                                   model::CallbackOrdinal::create_initial());
    if (!ordinal) {
      return model::Result<void>::create_failure(ordinal.error());
    }
    predicted = ordinal.value();
    auto fields =
        state_callback
            ? create_state_callback_trace_fields(grant, outcome.state_event().value(),
                                                 ordinal.value())
            : create_market_callback_trace_fields(grant, outcome.market_event().value(),
                                                  outcome.ready_book().value(), ordinal.value());
    auto nested_fields = create_reentry_trace_fields(grant, turn_ordinal.value(), ordinal.value());
    const auto kind = state_callback ? trace::RuntimeTraceEventKind::StateCallback
                                     : trace::RuntimeTraceEventKind::MarketCallback;
    auto valid = trace_sink_->validate_trace_record(kind, fields);
    if (!valid) {
      return valid;
    }
    nested_fields.failure_reason = trace::RuntimeTraceFailureReason::StrategyDispatchReentry;
    auto dispatch_reentry_valid = trace_sink_->validate_trace_record(
        trace::RuntimeTraceEventKind::ReentryDetected, nested_fields);
    ...
    auto owner_reentry_valid = trace_sink_->validate_trace_record(
        trace::RuntimeTraceEventKind::ReentryDetected, nested_fields);
  ...
  if (has_state) {
    for (std::size_t index = begin; index < end; ++index) {
      ...
      if (!prepared) {
        return model::Result<BotDispatchReport>::create_failure(prepared.error());
      }
    }
  }
  if (has_market) {
  ...
  for (auto& prepared : prepared_callbacks_) {
    const auto kind = prepared.state_callback ? trace::RuntimeTraceEventKind::StateCallback
                                              : trace::RuntimeTraceEventKind::MarketCallback;
    auto appended = trace_sink_->append_trace_record(kind, std::move(prepared.trace_fields));
    if (!appended) {
      status_.canonical_trace_failure_latched = true;
      dispatch_active_ = false;
      prepared_callbacks_.clear();
      return model::Result<BotDispatchReport>::create_failure(appended.error());
    }
    ...
    const auto started = measurement_clock_->processing_timestamp_now();
    ...
    const auto finished = measurement_clock_->processing_timestamp_now();
    context.deactivate();
  ...
  prepared_callbacks_.clear();
  return model::Result<BotDispatchReport>::create_success(report);
...
void BotRuntime::append_callback_diagnostic(RuntimeDiagnosticKind kind,
  ...
  auto appended = diagnostics_->append_diagnostic(kind, fields);
  if (!appended) {
