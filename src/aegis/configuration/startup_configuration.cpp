...
[[nodiscard]] constexpr std::uint16_t configuration_tag_code(ConfigurationTag value) noexcept {
...
[[nodiscard]] CanonicalConfigurationWriterResult
create_encoded_record(CanonicalConfigurationWriter writer, bool success) {
  if (!success) {
    return CanonicalConfigurationWriterResult::create_failure(
        DomainError::create_at_field(DomainErrorCode::EncodingOverflow, "configuration.encoding"));
  }
  return CanonicalConfigurationWriterResult::create_success(std::move(writer));
}

// --------------------------------------------------------
// Encode a firm record under its schema-1 field assignments.
[[nodiscard]] CanonicalConfigurationWriterResult encode_firm(const organization::Firm& firm) {
  CanonicalConfigurationWriter writer;
  const bool success = writer.append_ascii_field(1U, firm.id.value());
  return create_encoded_record(std::move(writer), success);
}

// --------------------------------------------------------
// Encode a desk record and its owning firm under fixed schema-1 field assignments.
[[nodiscard]] CanonicalConfigurationWriterResult encode_desk(const organization::Desk& desk) {
  CanonicalConfigurationWriter writer;
...
[[nodiscard]] CanonicalConfigurationWriterResult
encode_bot(const organization::BotRegistration& bot) {
  CanonicalConfigurationWriter writer;
...
[[nodiscard]] CanonicalConfigurationWriterResult
encode_strategy_settings(const BotStrategySettings& settings) {
  CanonicalConfigurationWriter writer;
...
[[nodiscard]] CanonicalConfigurationWriterResult encode_venue(const VenueDefinition& venue) {
  CanonicalConfigurationWriter writer;
...
[[nodiscard]] CanonicalConfigurationWriterResult
encode_logical_account(const LogicalAccountVenueBinding& binding) {
  CanonicalConfigurationWriter writer;
...
[[nodiscard]] CanonicalConfigurationWriterResult
encode_instrument_metadata(const model::InstrumentMetadata& metadata) {
  CanonicalConfigurationWriter writer;
  const bool success =
...
[[nodiscard]] CanonicalConfigurationWriterResult
encode_subscription(const market_data::Subscription& subscription) {
  CanonicalConfigurationWriter writer;
...
[[nodiscard]] CanonicalConfigurationWriterResult
encode_route(const execution::ExecutionRoute& route) {
  CanonicalConfigurationWriter writer;
...
encode_configuration(model::ConfigurationRevision revision,
  ...
  const bool success =
      ...
      writer.append_u64_field(configuration_tag_code(ConfigurationTag::ConfigurationRevision),
                              revision.value()) &&
      writer.append_u64_field(configuration_tag_code(ConfigurationTag::OrganizationRevision),
                              organization.revision().value()) &&
      writer.append_sequence_field(configuration_tag_code(ConfigurationTag::Firms),
                                   organization.firms(), encode_firm) &&
      writer.append_sequence_field(configuration_tag_code(ConfigurationTag::Desks),
                                   organization.desks(), encode_desk) &&
      writer.append_sequence_field(configuration_tag_code(ConfigurationTag::Bots),
                                   organization.bots(), encode_bot) &&
      writer.append_u64_field(
          configuration_tag_code(ConfigurationTag::StrategyConfigurationRevision),
          strategy_revision.value()) &&
      writer.append_sequence_field(configuration_tag_code(ConfigurationTag::StrategySettings),
                                   strategy_settings, encode_strategy_settings) &&
      writer.append_sequence_field(configuration_tag_code(ConfigurationTag::Venues), venues,
                                   encode_venue) &&
      writer.append_sequence_field(configuration_tag_code(ConfigurationTag::LogicalAccounts),
                                   logical_accounts, encode_logical_account) &&
      writer.append_sequence_field(configuration_tag_code(ConfigurationTag::InstrumentMetadata),
                                   instrument_metadata, encode_instrument_metadata) &&
      writer.append_u64_field(configuration_tag_code(ConfigurationTag::SubscriptionRevision),
                              subscriptions.revision().value()) &&
      writer.append_sequence_field(configuration_tag_code(ConfigurationTag::Subscriptions),
                                   subscriptions.subscriptions(), encode_subscription) &&
      writer.append_u64_field(configuration_tag_code(ConfigurationTag::RouteRevision),
                              routes.revision().value()) &&
      writer.append_sequence_field(configuration_tag_code(ConfigurationTag::Routes),
                                   routes.routes(), encode_route);
...
[[nodiscard]] bool has_venue(const std::vector<VenueDefinition>& venues,
...
[[nodiscard]] bool has_firm(const organization::Organization& organization,
...
validate_strategy_settings(std::vector<BotStrategySettings> settings,
  ...
  if (!duplicates) {
    return model::Result<std::vector<BotStrategySettings>>::create_failure(duplicates.error());
  }
  ...
  for (std::size_t index = 0U; index < bots.size(); ++index) {
    if (find_settings(settings, bots[index].id) == nullptr) {
      return model::Result<std::vector<BotStrategySettings>>::create_failure(
          DomainError::create_at_index(DomainErrorCode::InvalidRelationship,
                                       "strategy_settings.missing_bot", index));
    }
  }
  return model::Result<std::vector<BotStrategySettings>>::create_success(std::move(settings));
...
validate_venues(std::vector<VenueDefinition> venues) {
  ...
  if (venues.empty()) {
    return model::Result<std::vector<VenueDefinition>>::create_failure(
        DomainError::create_at_field(DomainErrorCode::EmptyCollection, "venues"));
  }
  ...
  if (!duplicates) {
    return model::Result<std::vector<VenueDefinition>>::create_failure(duplicates.error());
  }
  ...
  for (std::size_t index = 0U; index < venues.size(); ++index) {
    if (venues[index].environment != VenueEnvironment::Testnet) {
      return model::Result<std::vector<VenueDefinition>>::create_failure(
          DomainError::create_at_index(DomainErrorCode::InvalidValue, "venues.environment", index));
    }
  }
  return model::Result<std::vector<VenueDefinition>>::create_success(std::move(venues));
...
validate_logical_accounts(std::vector<LogicalAccountVenueBinding> bindings,
  ...
  if (!duplicates) {
    return model::Result<std::vector<LogicalAccountVenueBinding>>::create_failure(
        duplicates.error());
  }
  ...
  for (std::size_t index = 0U; index < bindings.size(); ++index) {
    if (!has_firm(organization, bindings[index].firm_id)) {
      return model::Result<std::vector<LogicalAccountVenueBinding>>::create_failure(
          DomainError::create_at_index(DomainErrorCode::DanglingReference,
                                       "logical_accounts.firm_id", index));
    }
    if (!has_venue(venues, bindings[index].venue_id)) {
      return model::Result<std::vector<LogicalAccountVenueBinding>>::create_failure(
          DomainError::create_at_index(DomainErrorCode::DanglingReference,
                                       "logical_accounts.venue_id", index));
    }
  }
  return model::Result<std::vector<LogicalAccountVenueBinding>>::create_success(
      std::move(bindings));
...
validate_instrument_metadata(std::vector<model::InstrumentMetadataParams> metadata_params,
  ...
  if (metadata_params.empty()) {
    return model::Result<std::vector<model::InstrumentMetadata>>::create_failure(
        DomainError::create_at_field(DomainErrorCode::EmptyCollection, "instrument_metadata"));
  }
  ...
  for (std::size_t index = 1U; index < metadata_params.size(); ++index) {
    if (metadata_params[index - 1U].venue_id == metadata_params[index].venue_id &&
      ...
      return model::Result<std::vector<model::InstrumentMetadata>>::create_failure(
          DomainError::create_at_index(DomainErrorCode::DuplicateIdentifier,
                                       "instrument_metadata.venue_instrument", index));
    }
  }
  ...
  // fields.
  using VenueNativeInstrumentKey = std::pair<model::VenueId, model::VenueInstrumentId>;
  std::set<VenueNativeInstrumentKey> native_instrument_keys;
  ...
  for (std::size_t index = 0U; index < metadata_params.size(); ++index) {
    ...
    if (!has_venue(venues, params.venue_id)) {
      return model::Result<std::vector<model::InstrumentMetadata>>::create_failure(
          DomainError::create_at_index(DomainErrorCode::DanglingReference,
                                       "instrument_metadata.venue_id", index));
    }
    ...
    auto validated = model::InstrumentMetadata::create_instrument_metadata(std::move(params));
    if (!validated) {
  ...
  return model::Result<std::vector<model::InstrumentMetadata>>::create_success(std::move(metadata));
...
StartupConfiguration::create_startup_configuration(StartupConfigurationParams params) {
  ...
  auto organization = organization::Organization::create_organization(
      params.organization_revision, std::move(params.firms), std::move(params.desks),
      std::move(params.bots));
  if (!organization) {
    return model::Result<StartupConfiguration>::create_failure(organization.error());
  }

  auto strategy_settings =
      validate_strategy_settings(std::move(params.strategy_settings), organization.value());
  if (!strategy_settings) {
    return model::Result<StartupConfiguration>::create_failure(strategy_settings.error());
  }

  auto venues = validate_venues(std::move(params.venues));
  if (!venues) {
    return model::Result<StartupConfiguration>::create_failure(venues.error());
  }

  auto logical_accounts = validate_logical_accounts(std::move(params.logical_accounts),
                                                    venues.value(), organization.value());
  if (!logical_accounts) {
    return model::Result<StartupConfiguration>::create_failure(logical_accounts.error());
  }

  auto metadata =
      validate_instrument_metadata(std::move(params.instrument_metadata), venues.value());
  if (!metadata) {
    return model::Result<StartupConfiguration>::create_failure(metadata.error());
  }
  ...
  auto subscriptions = market_data::SubscriptionConfiguration::create_subscription_configuration(
      params.subscription_revision, std::move(params.subscriptions), organization.value(),
      known_venue_instruments);
  if (!subscriptions) {
    return model::Result<StartupConfiguration>::create_failure(subscriptions.error());
  }

  // ++++++++++++++++++++++++++++++++++++++++
  // Route validation receives firm-owned accounts directly; no weaker post-validation is required.
  auto routes = execution::ExecutionRouteConfiguration::create_execution_route_configuration(
  ...
  if (!routes) {
    return model::Result<StartupConfiguration>::create_failure(routes.error());
  }
  ...
  if (!canonical_bytes) {
    return model::Result<StartupConfiguration>::create_failure(canonical_bytes.error());
  }

  // ++++++++++++++++++++++++++++++++++++++++
  // Hash those exact bytes once and bind the resulting identity to every contributing revision.
  ConfigurationFingerprint fingerprint{model::calculate_sha256_digest(canonical_bytes.value())};
  ...
  return model::Result<StartupConfiguration>::create_success(StartupConfiguration{
      params.revision,
