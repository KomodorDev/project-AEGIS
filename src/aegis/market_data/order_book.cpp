...
[[nodiscard]] model::Result<void> book_failure(model::DomainErrorCode code, std::string field) {
...
void FixedDepthOrderBook::clear() noexcept {
  bids_.clear();
  asks_.clear();
}
...
model::Result<void> FixedDepthOrderBook::apply(BookSide side, model::Price price,
  ...
  if (levels->size() >= retained_depth || levels->size() >= maximum_retained_book_depth) {
    return book_failure(model::DomainErrorCode::MarketBookCapacityExceeded,
...
model::Result<void> FixedDepthOrderBook::validate(const model::InstrumentMetadata& metadata,
  ...
  if (bids_.size() > retained_depth) {
    return book_failure(model::DomainErrorCode::MarketBookCapacityExceeded, "market_book.bids");
  }
  if (asks_.size() > retained_depth) {
    return book_failure(model::DomainErrorCode::MarketBookCapacityExceeded, "market_book.asks");
  }
  ...
  const auto validate_side = [&metadata](const std::vector<BookLevel>& levels, BookSide side) {
    for (const auto& level : levels) {
      if (level.price.coefficient() <= 0) {
        return book_failure(model::DomainErrorCode::MarketBookInvalid, side_price_field(side));
      }
      if (level.quantity.coefficient() <= 0) {
        return book_failure(model::DomainErrorCode::MarketBookInvalid, side_quantity_field(side));
      }
  ...
  if (bids_ordered != bids_.end()) {
    return book_failure(model::DomainErrorCode::MarketBookInvalid, "market_book.bids.order");
  }
  const auto asks_ordered = std::adjacent_find(
      asks_.begin(), asks_.end(),
      [](const BookLevel& lhs, const BookLevel& rhs) { return lhs.price >= rhs.price; });
  if (asks_ordered != asks_.end()) {
    return book_failure(model::DomainErrorCode::MarketBookInvalid, "market_book.asks.order");
  }
  ...
  if (!bids_.empty() && !asks_.empty() && bids_.front().price >= asks_.front().price) {
    return book_failure(model::DomainErrorCode::MarketBookInvalid, "market_book.crossed_or_locked");
  }
  return model::Result<void>::success();
