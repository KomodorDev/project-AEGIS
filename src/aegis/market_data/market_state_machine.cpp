...
struct TraceDrafts {
  ...
  void push(trace::RuntimeTraceEventKind kind, trace::RuntimeTraceFields fields) {
    entries[count] = Entry{kind, std::move(fields)};
    ++count;
  }

  // --------------------------------------------------------
  std::array<std::optional<Entry>, 2U> entries{};
  std::size_t count{0U};
};
...
// ########################################################################
// FanoutBudget fixes exact callbacks and the paired callback/re-entry trace slots derived from the
// actually published event set.
struct FanoutBudget {
...
[[nodiscard]] model::Result<void> failure(model::DomainErrorCode code, std::string field) {
  return model::Result<void>::failure(model::DomainError::at_field(code, std::move(field)));
}

// --------------------------------------------------------
// Lift a domain error into the result type needed by one public state-machine operation.
template <typename Value>
[[nodiscard]] model::Result<Value> propagate(const model::DomainError& error) {
  return model::Result<Value>::failure(error);
}

// --------------------------------------------------------
// Map the market boundary's four states to the independently assigned trace vocabulary.
[[nodiscard]] trace::RuntimeMarketState trace_state(MarketReadiness readiness) noexcept {
  switch (readiness) {
...
[[nodiscard]] model::Result<FanoutBudget> fanout_budget(std::uint32_t matching_subscriptions,
  ...
  if (callbacks > maximum_callbacks_per_turn ||
    ...
    return model::Result<FanoutBudget>::failure(model::DomainError::at_field(
  ...
  return model::Result<FanoutBudget>::success(FanoutBudget{
...
append_trace_drafts(TraceDrafts& drafts, model::MarketSourceOrdinal source_ordinal,
  ...
  for (std::size_t index = 0U; index < drafts.count; ++index) {
    auto validation =
        trace_sink.validate(drafts.entries[index]->kind, drafts.entries[index]->fields);
    if (!validation) {
      return validation;
    }
  }
  ...
  for (std::size_t index = 0U; index < drafts.count; ++index) {
    ...
    auto appended = trace_sink.append(draft.kind, std::move(draft.fields));
    if (!appended) {
      return appended;
    }
  }
...
require_initialized(const std::optional<MarketReadiness>& readiness) {
  if (!readiness) {
    return failure(model::DomainErrorCode::MarketNotReady, "market_state.initialization");
  }
  return model::Result<void>::success();
}
...
[[nodiscard]] model::Result<void> require_source(const MarketSourceIdentity& expected,
  ...
  if (expected != actual) {
    return failure(model::DomainErrorCode::RuntimeSourceNotConfigured, "market_state.source");
  }
  return model::Result<void>::success();
}
...
require_trace_provenance(const trace::RuntimeTraceProvenance& expected,
  ...
  if (trace_sink.provenance() != expected) {
    return failure(model::DomainErrorCode::InvalidRelationship, "market_state.trace_provenance");
  }
  return model::Result<void>::success();
}
...
[[nodiscard]] model::Result<void> require_processing_order(model::ProcessingTimestamp processing,
  ...
  auto delay = model::processing_delay(processing, receive);
  if (!delay) {
    return failure(model::DomainErrorCode::InvalidMarketEvent, "market_turn.processing_timestamp");
  }
  return model::Result<void>::success();
}
...
update_input_fields(const trace::RuntimeTraceSource& source, const NormalizedMarketUpdate& update,
  ...
  fields.state = trace_state(resulting_state);
...
[[nodiscard]] trace::RuntimeTraceFields
envelope_input_fields(const trace::RuntimeTraceSource& source, model::SessionEpoch session_epoch,
...
[[nodiscard]] trace::RuntimeTraceFields
discontinuity_input_fields(const trace::RuntimeTraceSource& source,
...
[[nodiscard]] trace::RuntimeTraceFields
update_transition_fields(const trace::RuntimeTraceSource& source,
                         const NormalizedMarketUpdate& update,
                         const AcceptedMarketTurnContext& context, MarketReadiness previous,
                         MarketReadiness next, const std::optional<BookIdentity>& identity) {
  auto fields = update_input_fields(
  ...
  fields.previous_state = trace_state(previous);
  return fields;
}

// --------------------------------------------------------
// Construct an accepted-envelope transition record for state-changing control input.
[[nodiscard]] trace::RuntimeTraceFields envelope_transition_fields(
  ...
  auto fields = envelope_input_fields(source, session_epoch, receive_sequence, context,
                                      trace::RuntimeInputDisposition::SessionReset, next, identity);
  fields.input_disposition = trace::RuntimeInputDisposition::Unspecified;
  fields.previous_state = trace_state(previous);
  return fields;
}
...
[[nodiscard]] trace::RuntimeTraceFields
owner_transition_fields(const trace::RuntimeTraceSource& source,
  ...
  fields.previous_state =
      previous ? trace_state(*previous) : trace::RuntimeMarketState::Unspecified;
  fields.state = trace_state(next);
  attach_book_identity(fields, identity);
  return fields;
}
...
[[nodiscard]] trace::RuntimeTraceFields
discontinuity_transition_fields(const trace::RuntimeTraceSource& source,
                                model::AdmissionOrdinal admission_ordinal,
                                model::TurnOrdinal turn_ordinal, MarketReadiness previous,
                                const std::optional<BookIdentity>& identity) {
  auto fields = discontinuity_input_fields(source, admission_ordinal, turn_ordinal,
...
update_state_event_fields(const MarketSourceIdentity& source, const NormalizedMarketUpdate& update,
                          const AcceptedMarketTurnContext& context, MarketReadiness previous,
                          MarketReadiness next, const std::optional<BookIdentity>& identity) {
...
[[nodiscard]] MarketStateEventFields
envelope_state_event_fields(const MarketSourceIdentity& source, model::SessionEpoch session_epoch,
                            model::ReceiveSequence receive_sequence,
...
[[nodiscard]] MarketStateEventFields
owner_state_event_fields(const MarketSourceIdentity& source, const OwnerMarketTurnContext& context,
                         std::optional<MarketReadiness> previous, MarketReadiness next,
                         const std::optional<BookIdentity>& identity) {
...
[[nodiscard]] MarketStateEventFields
discontinuity_state_event_fields(const MarketSourceIdentity& source,
                                 model::AdmissionOrdinal admission_ordinal,
...
model::Result<BookIdentity> BookIdentity::from_committed(model::BookGeneration generation,
  ...
  if (revision.value() < generation.value()) {
    return model::Result<BookIdentity>::failure(model::DomainError::at_field(
        model::DomainErrorCode::InvalidMarketEvent, "market_book.book_identity"));
  }
  return model::Result<BookIdentity>::success(BookIdentity{generation, revision});
}

// --------------------------------------------------------
// Advance both counters atomically in temporary values for a new authoritative snapshot.
model::Result<BookIdentity> BookIdentity::next_snapshot() const {
  auto generation = generation_.next();
  if (!generation) {
    return model::Result<BookIdentity>::failure(generation.error());
  }
  auto revision = revision_.next();
  if (!revision) {
    return model::Result<BookIdentity>::failure(revision.error());
  }
  return model::Result<BookIdentity>::success(
...
model::Result<BookIdentity> BookIdentity::next_delta() const {
  auto revision = revision_.next();
  if (!revision) {
    return model::Result<BookIdentity>::failure(revision.error());
  }
...
model::Result<void> validate_recorded_fixture_integrity(const NormalizedMarketUpdate& update,
  ...
  if (update.integrity().verdict != IntegrityVerdict::Accepted) {
    return failure(model::DomainErrorCode::MarketIntegrityFailure,
...
model::Result<MarketStateMachine>
MarketStateMachine::create(const runtime::RuntimePolicy& policy,
                           const runtime::RuntimeSource& source, model::InstrumentMetadata metadata,
  ...
  if (configured_source == nullptr || *configured_source != source) {
    return model::Result<MarketStateMachine>::failure(model::DomainError::at_field(
  ...
  return model::Result<MarketStateMachine>::success(MarketStateMachine{
...
MarketStateMachine::initialize(const OwnerMarketTurnContext& context,
  ...
  if (readiness_) {
    return model::Result<MarketTurnOutcome>::failure(model::DomainError::at_field(
  ...
  auto event_fields = owner_state_event_fields(source_, context, std::nullopt,
                                               MarketReadiness::Synchronizing, std::nullopt);
  auto event_validation = validate_market_state_transition(event_fields);
  if (!event_validation) {
    return propagate<MarketTurnOutcome>(event_validation.error());
  }
  TraceDrafts drafts;
  drafts.push(trace::RuntimeTraceEventKind::MarketStateTransition,
              owner_transition_fields(trace_source_, context, std::nullopt,
                                      MarketReadiness::Synchronizing, std::nullopt));
  auto fanout = fanout_budget(matching_subscription_count_, 1U, maximum_callbacks_per_turn_);
  if (!fanout) {
  ...
  return model::Result<MarketTurnOutcome>::success(
...
MarketStateMachine::resynchronize(const OwnerMarketTurnContext& context,
  ...
  if (!initialized) {
    return propagate<MarketTurnOutcome>(initialized.error());
  }
  ...
  TraceDrafts drafts;
  auto event_fields = owner_state_event_fields(source_, context, previous,
                                               MarketReadiness::Synchronizing, book_identity_);
  auto validation = validate_market_state_transition(event_fields);
  if (!validation) {
    return propagate<MarketTurnOutcome>(validation.error());
  }
  drafts.push(trace::RuntimeTraceEventKind::MarketStateTransition,
              owner_transition_fields(trace_source_, context, previous,
                                      MarketReadiness::Synchronizing, book_identity_));
  auto fanout = fanout_budget(matching_subscription_count_, 1U, maximum_callbacks_per_turn_);
  if (!fanout) {
    return model::Result<MarketTurnOutcome>::failure(fanout.error());
...
MarketStateMachine::process(NormalizedMarketUpdate update, const AcceptedMarketTurnContext& context,
  ...
  if (!source_valid) {
    return propagate<MarketTurnOutcome>(source_valid.error());
  }
  ...
  if (!time_valid) {
    return propagate<MarketTurnOutcome>(time_valid.error());
  }
  const auto previous = *readiness_;
  ...
  const auto finish_without_book = [&](trace::RuntimeInputDisposition disposition,
    ...
    drafts.push(
        ...
        update_input_fields(trace_source_, update, context, disposition, next, book_identity_));
    ...
    if (state_changes) {
      ...
      if (next == MarketReadiness::Synchronizing) {
        event_fields = envelope_state_event_fields(
            source_, update.session_epoch(), update.receive_sequence(), update.receive_timestamp(),
            context, previous, next, book_identity_);
        drafts.push(trace::RuntimeTraceEventKind::MarketStateTransition,
                    envelope_transition_fields(trace_source_, update.session_epoch(),
                                               update.receive_sequence(), context, previous, next,
                                               book_identity_));
      } else {
        event_fields =
            update_state_event_fields(source_, update, context, previous, next, book_identity_);
        drafts.push(trace::RuntimeTraceEventKind::MarketStateTransition,
                    update_transition_fields(trace_source_, update, context, previous, next,
                                             book_identity_));
      }
      auto validation = validate_market_state_transition(*event_fields);
      if (!validation) {
  ...
  if (update.kind() == MarketUpdateKind::Delta) {
    for (const auto& change : update.changes()) {
      if (change.quantity.coefficient() == 0) {
        auto applied =
            scratch_book_.apply(change.side, change.price, change.quantity, retained_depth_);
        if (!applied) {
  ...
  auto candidate_valid = scratch_book_.validate(metadata_, retained_depth_);
  if (!candidate_valid) {
    return finish_without_book(trace::RuntimeInputDisposition::StructuralBookRejected,
                               MarketReadiness::Invalid, false);
  }
  ...
  model::Result<BookIdentity> next_identity =
      update.kind() == MarketUpdateKind::Snapshot
          ? (book_identity_ ? book_identity_->next_snapshot()
                            : model::Result<BookIdentity>::success(BookIdentity::initial()))
          : book_identity_->next_delta();
  if (!next_identity) {
    return model::Result<MarketTurnOutcome>::failure(next_identity.error());
  }
  ...
  drafts.push(trace::RuntimeTraceEventKind::InputDisposition,
              update_input_fields(trace_source_, update, context, disposition,
                                  MarketReadiness::Ready, committed_identity));
  std::optional<MarketStateEventFields> state_fields;
  if (state_changes) {
    state_fields = update_state_event_fields(source_, update, context, previous,
                                             MarketReadiness::Ready, committed_identity);
    ...
    drafts.push(trace::RuntimeTraceEventKind::MarketStateTransition,
                update_transition_fields(trace_source_, update, context, previous,
                                         MarketReadiness::Ready, committed_identity));
  }
...
MarketStateMachine::process(const SessionStarted& control, const AcceptedMarketTurnContext& context,
  ...
  TraceDrafts drafts;
  drafts.push(trace::RuntimeTraceEventKind::InputDisposition,
              envelope_input_fields(trace_source_, control.session_epoch, control.receive_sequence,
  ...
  std::optional<MarketStateEventFields> event_fields;
  if (state_changes) {
    event_fields = envelope_state_event_fields(source_, control.session_epoch,
                                               control.receive_sequence, control.receive_timestamp,
                                               context, previous, next, book_identity_);
    auto validation = validate_market_state_transition(*event_fields);
    if (!validation) {
      return propagate<MarketTurnOutcome>(validation.error());
    ...
    drafts.push(trace::RuntimeTraceEventKind::MarketStateTransition,
                envelope_transition_fields(trace_source_, control.session_epoch,
...
model::Result<MarketTurnOutcome> MarketStateMachine::reject(
  ...
  TraceDrafts drafts;
  drafts.push(trace::RuntimeTraceEventKind::InputDisposition,
              envelope_input_fields(trace_source_, rejected.session_epoch,
                                    rejected.receive_sequence, context, disposition, next,
                                    book_identity_));
  ...
  if (state_changes) {
    ...
    drafts.push(trace::RuntimeTraceEventKind::MarketStateTransition,
                envelope_transition_fields(trace_source_, rejected.session_epoch,
...
model::Result<MarketTurnOutcome> MarketStateMachine::source_discontinuity(
  ...
  TraceDrafts drafts;
  drafts.push(trace::RuntimeTraceEventKind::InputDisposition,
              discontinuity_input_fields(trace_source_, failed_admission, context.turn_ordinal,
                                         next, book_identity_));
  std::optional<MarketStateEventFields> event_fields;
  if (state_changes) {
    event_fields = discontinuity_state_event_fields(source_, failed_admission, context, previous,
                                                    book_identity_);
    auto validation = validate_market_state_transition(*event_fields);
    if (!validation) {
      return propagate<MarketTurnOutcome>(validation.error());
  ...
  return model::Result<MarketTurnOutcome>::success(MarketTurnOutcome{
...
model::Result<ReadyBookView> MarketStateMachine::ready_book() const {
  if (readiness_ != MarketReadiness::Ready || !book_identity_) {
    return model::Result<ReadyBookView>::failure(model::DomainError::at_field(
        model::DomainErrorCode::MarketNotReady, "market_state.ready_book"));
  }
  return model::Result<ReadyBookView>::success(current_book_.ready_view());
}
